...
//			chatClient.sendMessage(MessageType.CONNECT+"|" + nickName);
			chatClient.sendMessage(MessageBuilder.build(MessageType.CONNECT, nickName));    //기존 위에꺼 만드는작업
	...
	public void appendNicknameChoice(String nickNameList) {
		userChoice.removeAll();   //다시 지우고 
		userChoice.add("전체에게");
		String[] nickNames = nickNameList.split("\n");
		for (String nickName : nickNames) {
			userChoice.add(nickName);  //for문 돌려서 추가
		}
	}
	 //메시지 보내기
...
//				chatClient.sendMessage(MessageBuilder.build(MessageType.CHAT_MESSAGE, nickName, message));
				...
				// DM 전송에 따른 분기      choice에 addItemListener는 필요없음.
				if(userChoice.getSelectedIndex() != 0) { //인덱스 0이 아니면 DM이라는 이야기
